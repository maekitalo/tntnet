...
std::unique_ptr<Worker> Worker::create(TntnetImpl& app)
{
    return std::unique_ptr<Worker>(new Worker(app));
}
...
void Worker::run()
{
                            ...
                            if (!socket.rdbuf()->in_avail())
                            {
                                ...
                                else
                                {
                                    ...
                                    if (::poll(&fd, 1, TntConfig::it().socketReadTimeout) == 0)
                                    {
                                        ...
                                        _application.getPoller().addIdleJob(std::move(j));
                                        keepAlive = false;
                                    }
                                }
                            }
    ...
    _application.workerFinished(this);
}
...
void Worker::logRequest(const HttpRequest& request, const HttpReply& reply, unsigned httpReturn)
{
    ...
    if (!accessLog.is_open())
    {
        ...
        std::lock_guard<std::mutex> lock(_application._accessLogMutex);
