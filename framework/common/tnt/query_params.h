...
class QueryParams : public cxxtools::QueryParams
{
    ...
    std::shared_ptr<Scope> _paramScope;
...
public:
    ...
    QueryParams(const QueryParams& src) = default;
    QueryParams& operator= (const QueryParams& src) = default;
    ...
    {
        if (!_paramScope)
            _paramScope = std::make_shared<Scope>();
        return *_paramScope;
    }
