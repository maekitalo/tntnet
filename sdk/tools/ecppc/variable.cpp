...
namespace tnt
{
  namespace ecppc
  {
    ...
    void Variable::getParamCode(std::ostream& o, const std::string& qparamparam) const
    {
      ...
      if (_isVector)
      {
        o << "std::vector< " << ltype << " > " << _name << " = request.getvector< " << ltype << " >(" << qparamparam << ", \"" << _name << "\");\n";
      }
      else if (_value.empty())
      {
        o << ltype << ' ' << _name << " = request.get< " << ltype << " >(" << qparamparam << ", \"" << _name << "\");\n";
      }
      else
      {
        o << ltype << ' ' << _name << " = request.get< " << ltype << " >(" << qparamparam << ", \"" << _name << "\", (" << _value << "));\n";
      }
    }
